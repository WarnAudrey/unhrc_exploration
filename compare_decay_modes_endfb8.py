...
output_path = '/Users/audreywarn/fluka-db-audrey/outputs/endf_endfb8_comparison.txt'
...
print("\n[1/4] Reading ENDF-B-VIII.0 data...")
endfb8_df = read_decay_data_with_modes(endfb8_path)
if endfb8_df is not None:
    endfb8_nuclides, endfb8_decay_data = extract_nuclides_and_modes(endfb8_df)
    print(f"Found {len(endfb8_nuclides)} unique nuclides in ENDF-B-VIII.0")
else:
    endfb8_nuclides, endfb8_decay_data = set(), {}
    print("Failed to read ENDF-B-VIII.0 data")

print("\n[2/4] Reading ENDF (JEFF-4.0) data...")
...
if endf_df is not None:
    ...
    print(f"Found {len(endf_nuclides)} unique nuclides in ENDF (JEFF-4.0)")
else:
    endf_nuclides, endf_decay_data = set(), {}
    print("Failed to read ENDF (JEFF-4.0) data")
...
print(f"  Total unique: {len(endfb8_nuclides | endf_nuclides)}")
...
with open(output_path, 'w') as f:
    ...
    f.write(f"\nDATA SOURCES:\n")
    f.write(f"  ENDF-B-VIII.0: {endfb8_path}\n")
    f.write(f"  ENDF (JEFF-4.0):  {endf_path}\n")
    
    f.write(f"\nNUCLIDE OVERLAP ANALYSIS:\n")
    f.write(f"  ENDF-B-VIII.0 nuclides: {len(endfb8_nuclides)}\n")
    f.write(f"  ENDF (JEFF-4.0) nuclides:  {len(endf_nuclides)}\n")
    f.write(f"  Common nuclides: {len(common_nuclides)}\n")
    f.write(f"  ENDF-B-VIII.0 only: {len(endfb8_only)}\n")
    f.write(f"  ENDF (JEFF-4.0) only: {len(endf_only)}\n")
    f.write(f"  Total unique: {len(endfb8_nuclides | endf_nuclides)}\n")
    
    # Print decay mode statistics for each category
    if endfb8_only:
        print_decay_mode_statistics(endfb8_only, endfb8_decay_data, "ENDF-B-VIII.0-ONLY NUCLIDES", f)
    
    if endf_only:
        print_decay_mode_statistics(endf_only, endf_decay_data, "ENDF (JEFF-4.0)-ONLY NUCLIDES", f)
    
    if common_nuclides:
        print_decay_mode_statistics(common_nuclides, endfb8_decay_data, "COMMON NUCLIDES (ENDF-B-VIII.0 data)", f)
        print_decay_mode_statistics(common_nuclides, endf_decay_data, "COMMON NUCLIDES (ENDF/JEFF-4.0 data)", f)
    
    # Overall statistics
    print_decay_mode_statistics(endfb8_nuclides, endfb8_decay_data, "ALL ENDF-B-VIII.0 NUCLIDES", f)
    print_decay_mode_statistics(endf_nuclides, endf_decay_data, "ALL ENDF (JEFF-4.0) NUCLIDES", f)
    ...
    endfb8_with_modes = sum(1 for n in endfb8_nuclides if n in endfb8_decay_data and endfb8_decay_data[n])
    endf_with_modes = sum(1 for n in endf_nuclides if n in endf_decay_data and endf_decay_data[n])
    
    if len(endfb8_nuclides) > 0:
        f.write(f"\nENDF-B-VIII.0: {endfb8_with_modes}/{len(endfb8_nuclides)} nuclides have decay mode data ({endfb8_with_modes/len(endfb8_nuclides)*100:.1f}%)\n")
    else:
        f.write(f"\nENDF-B-VIII.0: No nuclides found\n")
    
    if len(endf_nuclides) > 0:
        f.write(f"ENDF (JEFF-4.0):  {endf_with_modes}/{len(endf_nuclides)} nuclides have decay mode data ({endf_with_modes/len(endf_nuclides)*100:.1f}%)\n")
    else:
        f.write(f"ENDF (JEFF-4.0):  No nuclides found\n")
    ...
    if common_nuclides:
        ...
        both_have_modes = sum(1 for n in common_nuclides 
                              if n in endfb8_decay_data and endfb8_decay_data[n] 
                              and n in endf_decay_data and endf_decay_data[n])
        only_endfb8_has_modes = sum(1 for n in common_nuclides 
                                   if n in endfb8_decay_data and endfb8_decay_data[n] 
        ...
        only_endf_has_modes = sum(1 for n in common_nuclides 
                                  ...
                                  and (n not in endfb8_decay_data or not endfb8_decay_data[n]))
        neither_has_modes = len(common_nuclides) - both_have_modes - only_endfb8_has_modes - only_endf_has_modes
        
        f.write(f"  Both sources have mode data: {both_have_modes} ({both_have_modes/len(common_nuclides)*100:.1f}%)\n")
        f.write(f"  Only ENDF-B-VIII.0 has mode data: {only_endfb8_has_modes} ({only_endfb8_has_modes/len(common_nuclides)*100:.1f}%)\n")
        f.write(f"  Only ENDF (JEFF-4.0) has mode data: {only_endf_has_modes} ({only_endf_has_modes/len(common_nuclides)*100:.1f}%)\n")
