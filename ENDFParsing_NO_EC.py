...
try:
    ...
    print("Error: JEFF_ENDF_parser module not found. Please ensure it's in the same directory.")
    sys.exit(1)
...
class ENDFDataModule:
    ...
    def load_from_endf_files(self, endf_dir: str):
        """
        ...
        # Find all ENDF files (typically no extension or .endf)
        endf_files = sorted(list(endf_path.glob("*")))
        endf_files = [f for f in endf_files if f.is_file()]
        
        if not endf_files:
            raise ValueError(f"No ENDF files found in {endf_dir}")
...
def parse_endf_files(endf_dir: str) -> ENDFDataModule:
    """
    ...
    module.load_from_endf_files(endf_dir)
...
if __name__ == "__main__":
    ...
    # Parse ENDF files
    endf_module = parse_endf_files(args.endf_dir)
