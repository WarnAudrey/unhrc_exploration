        ...
        if np.any(inside):
            ...
            for k in range(len(self.breakpoints)):
                ...
                if itp == 1:
                    ...
                    y[inside][mask] = yi + (xk - xi) / (xi1 - xi) * (yi1 - yi)
                elif itp == 3:
                    y[inside][mask] = yi + np.log(xk / xi) / np.log(xi1 / xi) * (yi1 - yi)
                elif itp == 4:
                    y[inside][mask] = yi * np.exp((xk - xi) / (xi1 - xi) * np.log(yi1 / yi))
                elif itp == 5:
                    y[inside][mask] = yi * np.exp(
...
class ENDFDecayParser:
    ...
    Usage:
        ...
        decay_info = parser.parse_file("/Users/audreywarn/fluka-db-audrey/src/PyClasses/74Br.txt")
    """
